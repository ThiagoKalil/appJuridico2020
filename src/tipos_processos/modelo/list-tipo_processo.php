<?php
    ...
    if($conexao){
        ...
        if(!empty($requestData['draw']) || isset($requestData['draw'])){
            ...
            $sql = "SELECT idtipos_processo, nome, DATE_FORMAT(datamodificacao, '%d/%m/%Y %H:%i:%s') as datamodificacao, ativo FROM tipos_processos WHERE 1=1 ";
            ...
            if(!empty($filtro)){
                $sql .= " AND (idtipos_processo LIKE '$filtro%' ";
